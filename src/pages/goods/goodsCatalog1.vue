...
export default {
			...
			if(this.catalog == 1){
				...
				this.catalog = 2
				  var params = new URLSearchParams()
					...
					this.request.post('/goodsCatalog2/selectList', params).then(data => {
						this.catalog2List = data.retList
						this.currentPage = 1
						this.total = data.retList.length
					})
			}else{
				...
				this.retList.post('/goodsCatalog3/selectList', params).then(data => {
					this.catalog3List = data.retList
					this.currentPage = 1
					this.total = data.retList.length
				})
			}
  ...
  created() {
    //获取一级分类列表
    this.request.post('/goodsCatalog1/selectList').then(data => {
      this.catalog1List = data.retList
      this.loading = false
      this.total = data.retList.length
    })
  }
